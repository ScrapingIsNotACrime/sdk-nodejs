...
import type { BlueskyPost, BlueskyPosts, BlueskyProfile } from "../types/bluesky.js";
...
export class Bluesky {
  ...
  posts(handle: string, options: { limit?: number; cursor?: string } = {}): Promise<Page<BlueskyPost, BlueskyPosts>> {
    return cursorPage({
      cursor: options.cursor,
      load: (cursor) =>
        this.http.get<BlueskyPosts>(`/bluesky/profiles/${segment(handle)}/posts`, { limit: options.limit, cursor }),
